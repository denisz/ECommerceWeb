    ...
    return (
      <div id="outer-container" style={{height: '100%'}}>
        ...
        <main id="page-wrap">
          ...
          <a className={cx({'side-button': true, 'left': true, 'active': this.state.side === 'left'})}
             onClick={this.changeSide.bind(this, 'left')}>Left</a>
          <a className={cx({'side-button': true, 'right': true, 'active': this.state.side === 'right'})}
             onClick={this.changeSide.bind(this, 'right')}>Right</a>
          <nav className="demo-buttons">
            {Object.keys(this.props.menus).map((menu) => {
              return (
                <a key={menu}
                   className={cx({'current-demo': menu === this.state.currentMenu})}
