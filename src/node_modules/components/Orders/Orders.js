...
import {kDeliveryProviderBoxberry, kDeliveryProviderRussiaPost} from 'services/localizedDelivery'
...
export default class Orders extends QueryComponent {
  ...
  handleRowClick = (row) => {
    ...
    dialogs.showDialog(kDialogKey, {
      ...
      Component: <OrderEdit submitCancel
                            value={row}
                            disabled={row[keys.kStatusKey] === kOrderStatusDeclined}
  ...
  tableCellStyle(row) {
    if (row[keys.kDeliveryKey][keys.kDeliveryProviderKey] === kDeliveryProviderBoxberry) {
      return {
        backgroundColor: '#fff0f0',
      }
    }

    if (row[keys.kDeliveryKey][keys.kDeliveryProviderKey] === kDeliveryProviderRussiaPost) {
      return {
