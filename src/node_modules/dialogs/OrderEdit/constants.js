import localizeOrderStatus, {
  kOrderStatusAwaitingFulfillment,
  kOrderStatusAwaitingPayment,
  kOrderStatusAwaitingShipment,
  kOrderStatusAwaitingPickup,
  kOrderStatusDeclined,
  kOrderStatusRefunded,
  kOrderStatusShipped,
} from 'services/localizedOrderStatus';

export const KIdKey = 'id';
export const kStatusKey = 'status';
export const kReceiptNumberKey = 'receiptNumber';
export const kTrackingNumberKey = 'trackingNumber';
export const kNoticeRecipientKey = 'noticeRecipient';
export const kCommentKey = 'comment';

export const statuses = [
  kOrderStatusAwaitingPayment,
  kOrderStatusAwaitingFulfillment,
  kOrderStatusAwaitingPickup,
  kOrderStatusAwaitingShipment,
  kOrderStatusShipped,
  kOrderStatusRefunded,
  kOrderStatusDeclined,
].map(i => (
    {value: i, label: localizeOrderStatus(i), disabled: false, selected: false}
));