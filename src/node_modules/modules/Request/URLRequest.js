...
export default class URLRequest {
  constructor(url) {
    ...
    this[kURLParams] = {};
    this[kEncode] = 'json';
    this[kCredentials] = 'include';// 'include';
  }
  ...
  */
  attachAuthToRequest() {
    ...
    switch (authMethod) {
      ...
      case AuthMethods.JWT:
        if (payload.token) {
          this.appendHeader('Authorization', `Bearer ${payload.token}`);
        }
        break;
      default:
  ...
  async fetch() {
    if (URLRequest.xhr) {
      ...
      try {
        console.log(this.asCurl, `credentials: ${credentials}`);
        const result = await fetch(url, { method, headers, credentials, body });
