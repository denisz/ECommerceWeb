...
export default class Query extends AbstractQuery {
  ...
  appendUrlParams(...attrs) {
    this[kRequest].appendUrlParams(...attrs);
  }

  appendPayload(...attrs) {
    this[kRequest].appendPayload(...attrs);
  }

  limit(limit) {
    this.appendUrlParams({ limit });
  }

  skip(offset) {
    this.appendUrlParams({ offset });
  }

  page(page) {
    this.appendUrlParams({ page });
  }

  equalTo(key, val) {
    this.appendPayload({ [key]: val });
  }

  startsWith(key, val) {
    this.appendUrlParams({ [key]: val });
  }

  ascending(sort) {
    this.appendUrlParams({ sort: `${sort},asc` });
  }

  descending(sort) {
    this.appendUrlParams({ sort: `${sort},desc` });
  }
}
