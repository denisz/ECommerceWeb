...
export default class FormGroupValidate extends Component {
  ...
  render() {
    ...
    if (Array.isArray(error) && error.length > 0) {
      errorMessage = <span className="help-block invalid-feedback">{error.map(
          (e, i) => <span key={i}>{e}</span>)}</span>;
    } else if (isString(error)) {
      errorMessage = <span className="help-block invalid-feedback">{error}</span>;
    }
